...
rpn_json = model_rpn.to_json()
class_json = model_classifier.to_json()

with open("rpn.json", "w") as json_file:
	json_file.write(rpn_json)
...
for idx,img_name in enumerate(sorted(os.listdir(img_path))):
	...
	# Remove all but the box with the max probability of a class
	max_prob = 0
	for p in P_cls:
		for b in p:
			print(b)
			max_prob_pos = np.argmax(b)
			max_prob_each = b[max_prob_pos]
			if(max_prob_each>max_prob):
				max_prob = max_prob_each
	print(max_prob)
	cls_name = class_mapping[max_prob_pos]
	print(cls_name)
